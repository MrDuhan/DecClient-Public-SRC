...
import me.decclient.api.util.Wrapper;
import me.decclient.mod.Mod;
import me.decclient.mod.modules.Module;
import me.decclient.mod.modules.settings.Bind;
import me.decclient.mod.modules.settings.EnumConverter;
...
public class ConfigManager implements Wrapper {
    ...
    public String config = "dec/config/";
    ...
    public static void setValueFromJson(Mod mod, Setting setting, JsonElement element) {
        ...
        Dec.LOGGER.error("Unknown Setting type for: " + mod.getName() + " : " + setting.getName());
    }
    ...
    public void loadConfig(String name) {
        List<File> files = Arrays.stream(Objects.requireNonNull(new File("dec").listFiles())).filter(File::isDirectory).collect(Collectors.toList());

        if (files.contains(new File("dec/" + name + "/"))) {
            config = "dec/" + name + "/";
    ...
    public boolean configExists(String name) {
        List<File> files = Arrays.stream(Objects.requireNonNull(new File("dec").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("dec/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("dec/currentconfig.txt");

        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("dec", ""));
                writer.close();
    ...
    private void loadPath(Path path, Mod mod) throws IOException {
        ...
        try {
            ...
            Dec.LOGGER.error("Bad Config File for: " + mod.getName() + ". Resetting...");

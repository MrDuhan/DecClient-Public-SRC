...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (isListening) {
            Managers.TEXT.drawStringWithShadow(currentString.getString() + Managers.TEXT.getIdleSign(), x + 2.3f, y - 1.7f - (float) DecClickGui.INSTANCE.getTextOffset(), getState() ? -1 : -5592406);
        } else {
            Managers.TEXT.drawStringWithShadow((setting.getName().equals("Buttons") ? "Buttons " : (setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + setting.getValue(), x + 2.3f, y - 1.7f - (float) DecClickGui.INSTANCE.getTextOffset(), getState() ? -1 : -5592406);
        }
    }

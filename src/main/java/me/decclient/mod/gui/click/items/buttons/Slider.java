...
public class Slider extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (dotgod) {
            Managers.TEXT.drawStringWithShadow((getName().toLowerCase() + ":") + " " + ChatFormatting.GRAY + (setting.getValue() instanceof Float ? setting.getValue() : Double.valueOf(((Number) setting.getValue()).doubleValue())), x + 2.3f, y - 1.7f - (float) DecClickGui.INSTANCE.getTextOffset(), Managers.COLORS.getCurrentGui(240));

        } else {
            Managers.TEXT.drawStringWithShadow((newStyle ? getName().toLowerCase() + ":" : getName()) + " " + ChatFormatting.GRAY + (setting.getValue() instanceof Float ? setting.getValue() : Double.valueOf(((Number) setting.getValue()).doubleValue())), x + 2.3f, y - 1.7f - (float) DecClickGui.INSTANCE.getTextOffset(), -1);
        }
    }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        for (Component component : DecClickGui.INSTANCE.getComponents()) {

...
import me.decclient.api.managers.impl.ModuleManager;
import me.decclient.api.util.entity.EntityUtil;
...
import me.decclient.mod.modules.impl.combat.Aura;
import me.decclient.mod.modules.impl.combat.AutoTrap;
...
public class HUD extends Module {
    ...
    public Setting<String> watermarkString =
            add(new Setting<>("Text", "Dec", v -> !(mc.currentScreen instanceof DecClickGui || mc.currentScreen instanceof GuiMainMenu)));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (watermark.getValue()) {
            String decString = watermarkString.getValue() + " ";
            String verColor = watermarkVerColor.getValue() ? "" + ChatFormatting.WHITE : "";
            String verString = verColor + (watermarkShort.getValue() ? Dec.MODVER.substring(0, 4) : Dec.MODVER + "+" + Dec.VERHASH);

            if ((ClickGui.INSTANCE).rainbow.getValue()) {
                if ((ClickGui.INSTANCE).hudRainbow.getValue() == ClickGui.HudRainbow.STATIC) {
                    Managers.TEXT.drawString((lowerCase.getValue() ? decString.toLowerCase() : decString) + verString, 2.0F, waterMarkY.getValue(), Managers.COLORS.getRainbow().getRGB(), true);
                } else {
                    if (watermarkVerColor.getValue()) {
                        drawDoubleRainbowRollingString((lowerCase.getValue() ? decString.toLowerCase() : decString), verString, 2.0f, waterMarkY.getValue(), true);
                    } else {
                        Managers.TEXT.drawRollingRainbowString((lowerCase.getValue() ? decString.toLowerCase() : decString) + verString, 2.0f, waterMarkY.getValue(), true);
                    }
                }
            } else {
                Managers.TEXT.drawString((lowerCase.getValue() ? decString.toLowerCase() : decString) + verString, 2.0F, waterMarkY.getValue(), color, true);
            }
        }

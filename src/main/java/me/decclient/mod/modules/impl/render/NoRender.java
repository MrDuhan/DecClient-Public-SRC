...
public class NoRender
    ...
    public Setting<Boolean> fire = add(new Setting<Boolean>("Fire", Boolean.valueOf(false)));
    public Setting<Boolean> portal = add(new Setting<Boolean>("Portal", Boolean.valueOf(false)));
    public Setting<Boolean> pumpkin = add(new Setting<Boolean>("Pumpkin", Boolean.valueOf(false)));
    public Setting<Boolean> totemPops = add(new Setting<Boolean>("TotemPop", Boolean.valueOf(false)));
    public Setting<Boolean> items = add(new Setting<Boolean>("Items", Boolean.valueOf(false)));
    public Setting<Boolean> nausea = add(new Setting<Boolean>("Nausea", Boolean.valueOf(false)));
    public Setting<Boolean> hurtcam = add(new Setting<Boolean>("HurtCam", Boolean.valueOf(false)));
    public Setting<Fog> fog =add(new Setting<Fog>("Fog", Fog.NONE));
    public Setting<Boolean> noWeather = add(new Setting<Boolean>("Weather", Boolean.valueOf(false)));
    public Setting<Boss> boss = add(new Setting<Boss>("BossBars", Boss.NONE));
    public Setting<Float> scale = add(new Setting<Float>("Scale", (0.0f), (0.5f), (1.0f), v -> this.boss.getValue() == Boss.MINIMIZE || this.boss.getValue() != Boss.STACK));
    public Setting<Boolean> bats = add(new Setting<Boolean>("Bats", Boolean.valueOf(false)));
    public Setting<NoArmor> noArmor =add(new Setting<NoArmor>("NoArmor", NoArmor.NONE));
    public Setting<Boolean> glint = add(new Setting<Object>("Glint", Boolean.valueOf(false), v -> this.noArmor.getValue() != NoArmor.NONE));
    public Setting<Skylight> skylight = add(new Setting<Skylight>("Skylight", Skylight.NONE));
    public Setting<Boolean> barriers = add(new Setting<Boolean>("Barriers", Boolean.valueOf(false)));
    public Setting<Boolean> blocks = add(new Setting<Boolean>("Blocks", Boolean.valueOf(false)));
    public Setting<Boolean> advancements = add(new Setting<Boolean>("Advancements", false));
    public Setting<Boolean> pigmen = add(new Setting<Boolean>("Pigmen", false));
    public Setting<Boolean> timeChange = add(new Setting<Boolean>("TimeChange", false));
    public Setting<Integer> time = add(new Setting<Object>("Time", Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(23000), v -> this.timeChange.getValue()));
            ...
            if (this.boss.getValue() == Boss.MINIMIZE) {
                ...
                for (final Map.Entry<UUID, BossInfoClient> entry : map.entrySet()) {
                    ...
                    if (!event.isCanceled()) {
                        ...
                        NoRender.mc.fontRenderer.drawStringWithShadow(text, i / this.scale.getValue() / 2.0f - NoRender.mc.fontRenderer.getStringWidth(text) , (float) (j - 9), 16777215);
                    }
                ...
                for (final Map.Entry<String, Pair<BossInfoClient, Integer>> entry3 : to.entrySet()) {
                    ...
                    if (!event.isCanceled()) {
                        ...
                        NoRender.mc.fontRenderer.drawStringWithShadow(text, l / this.scale.getValue() / 2.0f - NoRender.mc.fontRenderer.getStringWidth(text), (float) (m - 9), 16777215);
                    }
                    GL11.glScaled(1.0 / this.scale.getValue(), 1.0 / this.scale.getValue(), 1.0);
